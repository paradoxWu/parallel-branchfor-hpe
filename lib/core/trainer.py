    ...
    def train(self):
        ...
        for i in range(self.num_iters_per_epoch):
            ...
            if target_2d and target_3d:
                ...
                inp2 = torch.cat((target_2d['kp_2d'], target_3d['kp_2d']), dim=0).cuda()
            elif target_3d:
                inp = target_3d['features'].to(self.device)
                inp2 = target_3d['kp_2d'].cuda()
            else:
                inp = target_2d['features'].to(self.device)
                inp2 = target_2d['kp_2d'].cuda()

            timer['data'] = time.time() - start
            start = time.time()

            preds = self.generator(inp,inp2,is_train=True)
    ...
    def validate(self):
        ...
        for i, target in enumerate(self.valid_loader):
            ...
            with torch.no_grad():
                inp = target['features']
                inp2 = target['kp_2d']
                preds = self.generator(inp, inp2,J_regressor=J_regressor)
    ...
    def fit(self):
        ...
        for epoch in range(self.start_epoch, self.end_epoch):
            ...
            if performance > 200.0:
                exit(f'MPJPE error is {performance}, higher than 200.0. Exiting!...')
