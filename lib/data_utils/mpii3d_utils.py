...
def read_data_train(dataset_path, debug=False):
    ...
    hrnet_yaml = '/home/wuyuanhao/code/VIBE/data/vibe_data/cls_hrnet_w64_sgd_lr5e-2_wd1e-4_bs32_x100.yaml'
    hrnet_checkpoint = '/home/wuyuanhao/code/VIBE/data/vibe_data/hrnetv2_w64_imagenet_pretrained.pth'
    update_config(hrnet_config, hrnet_yaml)
    model = get_cls_net(hrnet_config, pretrained=hrnet_checkpoint).to('cuda')
...
def mergedata():
    data1 = joblib.load('data/vibe_db/mpii3d_train_db1.pt')
    data2 = joblib.load('data/vibe_db/mpii3d_train_db2.pt')
    data3 = joblib.load('data/vibe_db/mpii3d_train_db3.pt')
    data4 = joblib.load('data/vibe_db/mpii3d_train_db4.pt')
    db_file = 'data/vibe_db/mpii3d_train_db.pt'
    ...
    print(f'Saving MPII-3d dataset to {db_file}')
    for k in dataset.keys():
        dataset[k] = np.concatenate((data1[k],data2[k],data3[k],data4[k]))
        print(f'{k}',dataset[k].shape)
    joblib.dump(dataset,db_file)
    print('Done!')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dir', type=str, help='dataset directory', default='/home/wuyuanhao/dataset/mpi_inf_3dhp')
    ...
    # joblib.dump(dataset, osp.join(VIBE_DB_DIR, 'mpii3d_train_db4.pt'))
    # mergedata()
